...
class BaiduUnitBot(Bot):
    def reply(self, query, context=None):
        ...
        url = f"https://aip.baidubce.com/rpc/2.0/unit/service/v3/chat?access_token={token}"
        post_data = (
        ...
        if response := requests.post(
            url, data=post_data.encode(), headers=headers
        ):
            return Reply(
                ReplyType.TEXT,
    ...
    def get_token(self):
        ...
        host = f"https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id={access_key}&client_secret={secret_key}"
